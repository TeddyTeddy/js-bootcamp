const Engine = (function todoFunctions () {
    ...
    const _getIncompleTodosCount = todos => {
        let count = 0
        todos.forEach(todo => {
            if(!todo.completed) {
                ++count
    ...
    const _filterTodos = (todos, filters) => {
    ...
    const renderTodos = (todos, filters) => {
        const filteredTodos = _filterTodos(todos, filters)
        ...
        const count = _getIncompleTodosCount(filteredTodos)
        h1DOM.textContent = `You have ${count} todo(s) left`
        todosDOM.appendChild(h1DOM)

        filteredTodos.forEach( todo => {
            const todoDOM = _generateTodoDOM(todo)
